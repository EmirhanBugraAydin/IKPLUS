...
public class RoleController {
	...
	public ResponseEntity<BaseResponse<List<RoleResponse>>> getAllRoles() {
		...
		return ResponseEntity.ok(BaseResponse.<List<RoleResponse>>builder().code(200).data(roleResponses)
		                                     .message("Roles listed successfully.").success(true).build());
	}
	...
	public ResponseEntity<BaseResponse<RoleResponse>> updateRole(@PathVariable Long id,
		...
		if (updatedRole == null) {
			return ResponseEntity.ok(BaseResponse.<RoleResponse>builder().code(400).data(null)
			                                     .message("Role not found.").success(false).build());
		}
		
		return ResponseEntity.ok(BaseResponse.<RoleResponse>builder().code(200).data(updatedRole)
		                                     .message("Role updated successfully.").success(true).build());
	}
	...
	public ResponseEntity<BaseResponse<RoleResponse>> getRoleById(@PathVariable Long id) {
		...
		Role role = roleService.findById(id).orElseThrow(() -> new IKPlusException(ErrorType.ROLE_NOT_FOUND));
		
		// 2. Adım: Eğer rol bulunamazsa, hata mesajı döndürüyoruz
		if (role == null) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
			                     .body(BaseResponse.<RoleResponse>builder().code(404).data(null)
			                                       .message("Rol bulunamadı.").success(false).build());
		}
		...
		return ResponseEntity.ok(BaseResponse.<RoleResponse>builder().code(200).data(roleResponse)
		                                     .message("Rol başarıyla getirildi.").success(true).build());
	}
	...
	public ResponseEntity<BaseResponse<Long>> getRoleIdByName(@RequestParam RoleName roleName) {
		...
		return ResponseEntity.ok(BaseResponse.<Long>builder().code(200).message("Role ID successfully fetched.")
		                                     .data(roleId).success(true).build());
	}
